...
def main():
    ...
    # 1. Load sentences
    sentences = load_sentences('../Step 2.1 Prepare Text for Word2Vec/2_1-processed_sentences.txt')

    # 2. Create bigrams
    sentences_with_ngrams = create_ngrams(sentences, save=True)

    # Save sentences with ngrams to a file
    with open('2_2-sentences_with_ngrams.txt', 'w', encoding='utf-8') as f:
        for sentence in sentences_with_ngrams:
            f.write(' '.join(sentence) + '\n')
    print("Sentences with bngrams saved to 2_2-sentences_with_ngrams.txt")
